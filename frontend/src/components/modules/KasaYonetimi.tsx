...
import { CalendarIcon, Wallet, Plus, Save, Download, Eye, Calculator, TrendingUp, TrendingDown, AlertCircle, DollarSign } from 'lucide-react';
...
export function KasaYonetimi() {
  ...
  const kasaHareketColumns = [
    ...
    },
    {
      ...
      header: "İşlem",
      cell: ({ row }: any) => (
        <Badge className={getIslemTipiColor(row.getValue("islemTipi"))}>
          {row.getValue("islemTipi") === 'Giris' ? 'Giriş' : 'Çıkış'}
        </Badge>
      )
    },
    {
    ...
    },
    {
      ...
      header: "Cari",
      cell: ({ row }: any) => {
        const cariAdi = row.getValue("cariAdi");
        const cariKodu = row.original.cariKodu;
        return cariAdi ? (
          <div>
            <div className="font-medium">{cariAdi}</div>
            <div className="text-sm text-muted-foreground">{cariKodu}</div>
          </div>
        ) : (
    ...
    },
    {
      ...
      header: "Tutar",
      cell: ({ row }: any) => {
        const tutar = row.getValue("tutar");
        const dovizTipi = row.original.dovizTipi;
        const islemTipi = row.original.islemTipi;
        return (
          <span className={islemTipi === 'Giris' ? 'text-green-600 font-medium' : 'text-red-600 font-medium'}>
            {islemTipi === 'Giris' ? '+' : '-'}
            {formatCurrency(tutar, dovizTipi)}
          </span>
        );
      }
    },
    {
      ...
      header: "Durum",
      cell: ({ row }: any) => (
        <Badge className={getDurumColor(row.getValue("onayDurumu"))}>
          {row.getValue("onayDurumu")}
        </Badge>
      )
    }
  ];
    ...
    <div className="h-full flex flex-col p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card className="p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-green-600">$26,200</p>
        ...
        <Card className="p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-orange-600">$18,350</p>
        ...
        <Card className="p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-blue-600">$7,850</p>
        ...
        <Card className="p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-muted-foreground">Bekleyen Onaylar</p>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="h-full">
        ...
        <TabsContent value="hareket-listesi" className="flex-1">
          <UltraProfessionalTable
            data={kasaHareketleri}
            columns={kasaHareketColumns}
            title="Kasa Hareketleri"
            description="Tüm nakit giriş ve çıkış işlemlerini görüntüleyin"
            searchPlaceholder="Kategori, cari adı veya açıklama ile ara..."
            enableSearch
            enableFilters
            enableExport
            ...
            showMetrics
            metrics={{
              total: kasaHareketleri.length,
              active: kasaHareketleri.filter(k => k.onayDurumu === 'Onaylandi').length,
              pending: kasaHareketleri.filter(k => k.onayDurumu === 'Beklemede').length,
              completed: kasaHareketleri.filter(k => k.islemTipi === 'Giris').length
            }}
          />
        </TabsContent>
